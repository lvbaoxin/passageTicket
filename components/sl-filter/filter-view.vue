				...
				<view v-if="item.isSort">
					<view class="filter-content-list">
						<view v-for="(detailItem,idx) in selectDetailList" :key="idx"
							...
							:style="{'color': detailItem.isSelected?themeColor:'#1A1A1A'}"
				...
				<view v-else>
					...
					<view class="filter-content-detail">
						<text v-for="(detailItem,idx) in selectDetailList" :key="idx"
							...
							:class="{'filter-content-active':detailItem.isSelected? 'cs': ''}"
							:style="{'border-color':detailItem.isSelected?themeColor:'#1485ee','color':detailItem.isSelected?'#1485ee':'#1A1A1A','border':'1px','border-style':'solid',}"
							@tap="itemTap(idx,selectDetailList,item.isMutiple,item.key)">
							{{detailItem.title}}
						</text>
					</view>
					<view class="filter-content-footer">
						<view class="filter-content-footer-item" style="color: #A8A8A8; background-color: #FFFFFF;"
